...
pub struct SqliteBlob<'conn> {
    pub(crate) blob: std::ptr::NonNull<ffi::sqlite3_blob>,
    pub(crate) read_index: usize,

    pub(crate) blob_size: usize,
    pub(crate) _pd: std::marker::PhantomData<&'conn mut ffi::sqlite3_blob>,
}
...
fn to_io_error(error: std::num::TryFromIntError) -> std::io::Error {
...
impl std::io::Read for SqliteBlob<'_> {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        ...
        // from the blob.
        let read_length: i32 = (i32::try_from(self.blob_size).map_err(to_io_error)? - offset).min(buflen);

        let ret = unsafe {
            ffi::sqlite3_blob_read(self.blob.as_ptr(), buf.as_mut_ptr() as *mut std::ffi::c_void, read_length, offset)
        };
        ...
        Ok(usize::try_from(read_length).map_err(to_io_error)?)
    }
}
...
impl std::io::Seek for SqliteBlob<'_> {
    fn seek(&mut self, pos: std::io::SeekFrom) -> std::io::Result<u64> {
        ...
        Ok(u64::try_from(self.read_index).map_err(to_io_error)?)
    }
}
